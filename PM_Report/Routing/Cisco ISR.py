...
class CiscoLogParser:
    ...
    def model_number(self):
        chassis_section = re.search(r"NAME: \"Chassis\".*?(?=NAME:)", self.log_data, re.DOTALL)
        if chassis_section:
            match = re.search(r" is PID:\s+(\S+)", chassis_section.group(0))
            return match.group(1) if match else "NA"
        return "NA"
    ...
    def serial_number(self):
        chassis_section = re.search(r"NAME: \"Chassis\".*?(?=NAME:)", self.log_data, re.DOTALL)
        if chassis_section:
            match = re.search(r"is SN:\s+(\S+)", chassis_section.group(0))
            return match.group(1) if match else "NA"
        return "NA"
    ...
    def current_sw_version(self):
        match = re.search(r"Cisco IOS XE Software, Version\s+([\d]+\.[\d]+\.[\d]+[a-zA-Z]*\.?[a-zA-Z]*)", self.log_data)
        return match.group(1) if match else "NA"
    ...
    def flash_info(self):
        match = re.search(r"(\d+)\s+bytes\s+available\s+\((\d+)\s+bytes\s+used\)", self.log_data)
        if match:
            used = int(match.group(2))
