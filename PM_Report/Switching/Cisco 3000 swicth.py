        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                for k, v in value.items():
                    print(f"  {k}: {v if v is not None else 'NA'}")
            else:
                print(f"{key}: {value if value is not None else 'NA'}")
        print("\n")


def main():
    # For a single file
    file_path = r"Log_samples\3650 switch.txt"
    parser = CiscoLogParser(file_path)
    data = {
        "Hostname": parser.hostname(),
        "Model number": parser.model_number(),
    ...
    # For a directory
    # directory_path = r"Log_samples\3650 switch.txt"
