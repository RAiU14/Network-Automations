...
def _validate_request(ticket: str, file_obj, technology: str, allowed_extensions: set) -> Dict[str, Any]:
    """Validate the upload request"""
    errors = []
    
    if not ticket:
        errors.append('Ticket Number is required.')
    elif not validate_ticket_number(ticket):
    ...
    elif not allowed_file(file_obj.filename, allowed_extensions):
...
def _setup_ticket_folder(upload_folder: str, ticket: str) -> Path:
    """Create and setup ticket folder"""
    upload_path = Path(upload_folder)
    ticket_folder = upload_path / ticket
...
def _save_uploaded_file(file_obj, ticket: str, ticket_folder: Path) -> Path:
...
def _create_metadata(ticket: str, comment: str, technology: str, 
...
def _save_metadata(metadata: Dict[str, Any], meta_path: Path) -> None:
...
def _process_file(file_path: Path, ticket_folder: Path, ticket: str, 
            ...
            if modules['Cisco_IOS_XE'] and hasattr(Cisco_IOS_XE, 'process_directory'):
                ...
                try:
                    ...
                    if modules['Data_to_Excel']:
                        excel_result = _process_excel(data, ticket, ticket_folder, technology, metadata, modules)
...
def _process_excel(data: Any, ticket: str, ticket_folder: Path, 
                  technology: str, metadata: Dict[str, Any], modules: Dict[str, bool]) -> Dict[str, Any]:
...
def process_upload(request_data: Dict[str, Any], file_obj, upload_folder: str, allowed_extensions: set) -> Dict[str, Any]:
    ...
    try:
        ...
        # Validation
        validation_result = _validate_request(ticket, file_obj, technology, allowed_extensions)
        ...
        ticket_folder = _setup_ticket_folder(upload_folder, ticket)
        file_path = _save_uploaded_file(file_obj, ticket, ticket_folder)
        
        # Create and save metadata
        metadata = _create_metadata(ticket, comment, technology, file_path, file_obj.filename, modules)
        meta_path = ticket_folder / 'metadata.json'
        _save_metadata(metadata, meta_path)
        
        # Process the file
        logging.info(f"Starting file processing for {file_path}")
        processing_result = _process_file(file_path, ticket_folder, ticket, technology, metadata, modules)
...
# Configuration storage for app.py
_CONFIG = {
    'upload_folder': None,
    'allowed_extensions': None
}

def initialize_controller(upload_folder: str, allowed_extensions: set) -> None:
    """Initialize controller configuration"""
    global _CONFIG
    _CONFIG['upload_folder'] = upload_folder
    _CONFIG['allowed_extensions'] = allowed_extensions
    
    # Ensure upload folder exists
    upload_path = Path(upload_folder)
    upload_path.mkdir(parents=True, exist_ok=True)
    logging.info(f"Controller initialized with upload folder: {upload_folder}")

def get_upload_folder() -> str:
    """Get configured upload folder"""
    return _CONFIG['upload_folder']

def get_allowed_extensions() -> set:
    """Get configured allowed extensions"""
    return _CONFIG['allowed_extensions']

# Factory function for backward compatibility
def get_controller(upload_folder: str, allowed_extensions: set) -> Dict[str, Any]:
    """Factory function to initialize controller configuration"""
    logging.info(f"Initializing controller with folder: {upload_folder}, extensions: {allowed_extensions}")
    initialize_controller(upload_folder, allowed_extensions)
    
    # Return a dict with the main functions
    return {
        'process_upload': lambda request_data, file_obj: process_upload(request_data, file_obj, upload_folder, allowed_extensions),
        'check_module_availability': check_module_availability,
        'validate_ticket_number': validate_ticket_number,
        'allowed_file': lambda filename: allowed_file(filename, allowed_extensions)
    }
