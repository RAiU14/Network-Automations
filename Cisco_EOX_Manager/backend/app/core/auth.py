...
def auth_enabled() -> bool:
    return os.getenv("EOX_AUTH_ENABLED", "false").strip().lower() not in {"0", "false", "no", "off"}
...
def save_admin_token(token: str) -> AuthStatus:
    ...
    path.write_text(
        ...
        f"EOX_AUTH_UPDATED_AT={updated_at}\n"
        f"EOX_ADMIN_TOKEN_HASH={_hash_token(clean)}\n",
        encoding="utf-8",
    )
...
class AdminAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if verify_token(extract_token(request)):
            return await call_next(request)
        return JSONResponse({"detail": "Admin token required"}, status_code=status.HTTP_401_UNAUTHORIZED)
